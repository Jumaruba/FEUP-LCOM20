...
int (timer_set_frequency)(uint8_t timer, uint32_t freq) {
  ...
  //Then, we have that the minimum value for the frequence is 19. 
  if (freq < 19){
    //printf("ERR: timer_set_frequency --freq** freq out of range\n");
    return 7;
  }
...
int (timer_subscribe_int)(uint8_t *bit_no) {
  ...
  *bit_no = BIT(hook_id); 
  ...
  /*//if this code snipet gets uncommented, the compiler returns an error 
  because irqsetpolicy() have already set the bit to 1
